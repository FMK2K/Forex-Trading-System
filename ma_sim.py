...
def run():
    currencies = "GBP,EUR,CAD,JPY,NZD,CHF"
    granularity = "H1"
    ...
    for pairname in test_pairs:
        ...
        try:
            ...
            print(e)
            print(f"Skipping {pairname} due to missing data file.")
            continue
...
#Note that the error indicates that `utils.get_instruments_data_filename`
#is being treated as a class method but is not defined as such. To resolve this,
#ensure that `get_instruments_data_filename` is defined as a static method or a 
#regular function in `utils.py`, and adjust the call accordingly in `instrument.py`.
#Also, check for any naming conflicts or import issues that may be causing the incorrect usage.
